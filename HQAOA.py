...
if run_args.GPU==True:
    qem.sync()
run_args.end=time() 
run_args.wall_clock=(run_args.end-run_args.start)/60/60 # Wall-clock time of QAOA run, in hours.
####################
...
if cmd_args.direction=='temporal':
    run_args.inf_qaoa=_HQAOA.infidelity_from_parameters_swap_temporal_quantum_classical(run_args.con,run_args.n,cmd_args.d,cmd_args.p,cmd_args.ep,_opt_par,run_args.ground_space)
elif cmd_args.direction=='spatial':
    run_args.inf_qaoa=_HQAOA.infidelity_from_parameters_swap_spatial(run_args.con,run_args.n,cmd_args.d,cmd_args.p,cmd_args.ep,_opt_par,run_args.ground_space)
...
if not os.path.exists(cmd_args.path+'/output.txt'):
    ...
    f.write("### Output of the QAOA runs which have the folder this txt file is in as 'path' argument.\n")
