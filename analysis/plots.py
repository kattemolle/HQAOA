...
def plot_points(unaware_path, path, instance, d, n_iter, ep, n):
    ...
    np.save(
        "plot_points/p_plot_plot_points_noise_aware_{}.npy".format(
...
def plot_points_noise_unaware(
    ...
    p_list = get_p_list(
        unaware_path, instance, d, n_iter, ep, n
    ...
    np.save(
        "plot_points/p_plot_plot_points_noise_unaware_{}.npy".format(
...
def plotter(
    ...
    fname = "plot_points/p_plot_plot_points_noise_aware_{}.npy".format(
    ...
    opt_unaware_pars = _HQAOA.load_optimal_line(unaware_path + instance, 0, 0, d)[2][
        "opt_parameters"
    ]  # Load optimal parameters at p,ep=0.
    ...
    fname = "plot_points/p_plot_plot_points_noise_unaware_{}.npy".format(
        hash((unaware_path, instance, n, d, n_iter, direc, ep))
    )
    if not os.path.isfile(fname):
        fname = "plot_points/p_plot_plot_points_noise_unaware_{}.npy".format(
...
def get_ep_list(
    path, instance, d, n_iter, p, n
):  # Get the list of available ep values in path+instance at the given d,n_iter and p. Sort the list.
...
def kappa_plot_points(path, instance, d, n_iter, p, n):
    ...
    np.save(
        "plot_points/kappa_plot_plot_points_noise_aware_{}.npy".format(
            hash((path, instance, d, n_iter, p))
        ),
        pltpts,
        allow_pickle=True,
    )
...
def kappa_plot_points_noise_unaware(
    unaware_path, instance, n, d, n_iter, direc, p
):  # Take the optimal angles at p=0, and use them to obtain an AR at other p.
    pltpts = []
    E0 = _HQAOA.get_instance_E0(unaware_path + instance)
    con = _HQAOA.get_instance_con(unaware_path + instance)
    ep_list = get_ep_list(
        unaware_path, instance, d, n_iter, p, n
    )  # get the ep for which I ran noiseless optimization.
    with ch.no_backprop_mode():
    ...
    np.save(
        "plot_points/kappa_plot_plot_points_noise_unaware_{}.npy".format(
            hash((unaware_path, instance, n, d, n_iter, direc, p))
        ),
        pltpts,
    )
    return pltpts
...
def kappa_plotter(
    ...
    fname = "plot_points/kappa_plot_plot_points_noise_aware_{}.npy".format(
        hash((direc_path, instance, d, n_iter, p))
    )
    if not os.path.isfile(fname):
        fname = "plot_points/kappa_plot_plot_points_noise_aware_{}.npy".format(
            hash(("../../" + direc_path, instance, d, n_iter, p))
    ...
    fname = "plot_points/kappa_plot_plot_points_noise_unaware_{}.npy".format(
        hash((unaware_path, instance, n, d, n_iter, direc, p))
    )
    if not os.path.isfile(fname):
        fname = "plot_points/kappa_plot_plot_points_noise_unaware_{}.npy".format(
    ...
    if os.path.isfile(fname):
        ...
        print("Calculating plot points")
        points = kappa_plot_points_noise_unaware(
            unaware_path, instance, n, d, n_iter, direc, p
        )
...
def meta_plotter(instance):
    fig, ax = plt.subplots(
        8,
        figsize=(3.75, 12),
        gridspec_kw={"height_ratios": [2, 1, 2, 1, 1, 0.5, 1, 0.5]},
    )
    ...
    fig.tight_layout()
    fig.savefig(
        "plots/plot_{}.pdf".format(instance), bbox_inches="tight", pad_inches=0.005
    )
