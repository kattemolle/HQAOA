...
def error_locs_temporal(n,d): # Take spatial noise locs, pad the lines with lower length with None to make regular array, transpose the array, remove the Nones.  
...
def calc_linearized_AR(case): # Case should specify: case.ipath_noiseless,case.ipath_noise,n,case.load_p,case.load_ep,case.d,case.error_locs_fn,case.noise_aware_p,case.noise_aware_ep
    ...
    for error_loc in case.error_locs_fn(case.n,case.d):
        term=_HQAOA.cost_from_parameters_swap_spatial(con,case.n,case.d,inj_p,compute_ep,pars_noiseless,error_loc).data # Note the spatial cost_from_parameters is used in all cases. It's ok because errors are inserted manually at compute_ep=0 with p=1 anyway.
        out.av+=term
    ...
    out.noise_aware_cost=_HQAOA.load_optimal_line(case.ipath_noise,case.noise_aware_p,case.noise_aware_ep,case.d)[2]['cost_qaoa']
    out.av=out.av/l 
    ...
    out.rel_error_with_aware=str((out.noise_aware_AR-out.extrap_AR)/out.noise_aware_AR*100)+'%'
    
    # Also compute noise unwarare AR for comparison
    if 'spatial' in case.ipath_noise:
        out.noise_unaware_AR=_HQAOA.cost_from_parameters_swap_spatial(con,case.n,case.d,case.noise_aware_p,case.noise_aware_ep,pars_noiseless).data 
    elif 'temporal' in case.ipath_noise:
        out.noise_unaware_AR=_HQAOA.cost_from_parameters_swap_temporal_quantum_classical(con,case.n,case.d,case.noise_aware_p,case.noise_aware_ep,pars_noiseless).data
    out.noise_unaware_AR=out.noise_unaware_AR/E0
    out.rel_error_with_unaware=str((out.noise_unaware_AR-out.extrap_AR)/out.noise_unaware_AR*100)+'%'
...
if __name__=='__main__':
    ...
    # Run
    list(map(calc_linearized_AR,[temporal,spatial,uncor,temporal2,spatial2,uncor2]))
