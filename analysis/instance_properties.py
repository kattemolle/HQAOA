...
class GraphIt: # Create init graph. The __next__ method returns the next SK graph in a certain sequence. 
    ...
    def i_to_bstr(self,i): # Turn the integer i to a bitstring in the form of an iterator. Also output its hamming weight. The bitstring is of the form e.g. [-1,1,1,...] and represents a SK graph by only specifying weights.
        get_bin=lambda x: format(x, 'b').zfill(self.ned) # Function that outputs integers in the binary representation as a string.
        st=get_bin(i)
    ...
    def bstr_to_graph(self,bstr,ham_weight):
        edges=[(i,j,next(bstr)) for i in range(self.ned) for j in range(i+1,self.n)]
        g=nx.Graph()
...
def compute_classes(n): # Compute the classes, send Hamming weights and multiplicities to st out, save graphs, Hamming weights and multiplicities to disk.
    ...
    for g,ham_weight in gi:
        ...
        iso=False
        for j,tup in enumerate(classes): # Check if the current graph is isomorphic to any of the previously encountered class representatives.
            if ham_weight==tup[1]:
                if nx.is_isomorphic(tup[0],g,edge_match=edge_match):
                    classes[j][2]+=1
                    iso=True
                    break
        if iso==False: # If it was not isomorphic to any of the class representatives, add the graph to the list of representatives.
    ...
    print(meta)
    assert sum(all_mult)==2**ned, 'Graph multiplicities do not add up to total numer of SK graphs.'

    # Save to disk.
    name='SK_graph_classes_n={}.dat'.format(n)
    with open(name, 'wb') as file:
        pickle.dump(classes,file,protocol=4)
    print('Wrote graph classes to', name, 'in the format [[nx.Graph,ham_weight,mult],...]')
            ...
            for p in p_list:
                for ep in ep_list:
                    for par_i,pars in enumerate(randvecs):
                        ...
                        output=str([direc,inst,p,ep,pars.tolist(),float(cost),par_i])
                        if not os.path.exists('landscape_data.txt'):
                            with open('landscape_data.txt', 'a') as f:
                                f.write("### The value of the cost function for various instances and points. Syntax [instance,direction,p,ep,parameters,cost,parameters_id]\n")
                        with open('landscape_data.txt', 'a') as f:
                            f.write(output+'\n\n')
                            print(output)
...
def pm_instance_strings():
    ...
    dirsbin=[di for di in dirsbin if os.path.isdir('../data/SK/SWAP-network/6/spatial/'+di)]
    dirsbin.sort()
...
def get_class_number(classes,instance): # Get class index (as def by classes) of the graph with weights instance (in +/- notation).
...
def import_landscape_data():
    ...
    dirs=[path+inst for inst in sorted(os.listdir(path)) if os.path.isdir(path+inst)]
    dirs.sort()
    per_inst=[]
    for di in dirs:
...
def dists(p):
    ...
    for i in range(len(cv)):
        for j in range(i+1,len(cv)):
            dif_vec=cv[i]-cv[j]
            max_dist=max(np.abs(dif_vec)) #select the point with the max dist in abs val.
...
def small_dists(p):
    ...
    if len(sd)!=0:
        print('Largest element-wise dist between the cost vecs of two instances in the same cost function class:')
...
def load_ground_space(instancebin): # Load the ground space data of the instance. Instance should be given as string in 0/1 notation. Output is a tuple containing the ground space energy and the ground space.
    with open('../data/SK/SWAP-network/6/spatial/'+str(instancebin)+'/ground_state.txt','r') as f:
        a=f.readlines()
...
if __name__=='__main__':
    ...
    classes.sort(key=lambda x: x[2:0:-1]) # Sort classes by multiplicity and then by ham weight.
    ...
    g=nx.Graph()
    g.add_edges_from(small_dists(0)[:,:2]) # Construct graph where instances are nodes. There is an edge between them if the max element-wise dist of their cost vecs is below a threshold value. Then the connected components of this graph are the cost function classes.

    print('The cost function landscape classes are',list(nx.connected_components(g)),',')
    print('where instances are represented by their lexcicographical order, and excluing landscape classes with a single member.')
    ...
    print('Smallest element-wise dist between the cost vecs of two instances in different cost function classes:')
    print(min(large_dists.transpose()[2]))
    print()

    # Gather all other data and print in table
    data=[['Inst. bitstr.','lex. ord.','gr. cl. ind.','gr. cl. mult.','gs. en.','gs']]
    cst_fn_cl=0
