...
def plot_points(path, instance, d, n_iter, p):
    ...
    np.save(
        "plot_points/lone_kappa_plot_plot_points_noise_aware_{}.npy".format(
...
def plot_points_noise_unaware(
    ...
    ep_list = get_ep_list(
        unaware_path, instance, d, n_iter, p
    ...
    np.save(
        "plot_points/lone_kappa_plot_plot_points_noise_unaware_{}.npy".format(
...
def plotter(
    direc_path, direc, instance, unaware_path, n_iter, d, p, label1, label2, ax, _color
):
    # If you made new data points, the npy files should be removed. Only in that case the plot points are computed anew.
    fname = "plot_points/lone_kappa_plot_plot_points_noise_aware_{}.npy".format(
    ...
    fname = "plot_points/lone_kappa_plot_plot_points_noise_unaware_{}.npy".format(
        hash((unaware_path, instance, n, d, n_iter, direc, p))
    )
    if os.path.isfile(fname):
        points = np.load(fname)
...
if __name__ == "__main__":
    ...
    pts = np.array(
        [temp_AR, spat_AR, uncor_AR, temp2_AR, spat2_AR, uncor2_AR]
    ).transpose()
    lin = ax.scatter(
        *pts,
        marker="x",
