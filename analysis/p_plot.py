...
os.environ["PYTHONHASHSEED"]="0" # To get the same hash values every time new python process is started.
rc('font', **{'family': 'serif', 'serif': ['Computer Modern'], 'size': 9})
rc('text', usetex=True)

def get_p_list(path,instance,d,n_iter,ep): # Get the list of available p values in path+instance at the given d,n_iter and ep. Sort the list.
...
def plot_points(unaware_path,path,ipath,d,n_iter,ep):
    ...
    for p in p_list:  # Per p, take the line with the smallest cost
        if p==0: # For p=0, allways look in the noise_unaware folder, because at p=0, results are the same for any ep and at p=0 I did not recompute for every ep.
            best_cost=_HQAOA.load_optimal_line(unaware_path+instance, p, 0, d)[2]['cost_qaoa']
        else:
    ...
    np.save('plot_points/lone_p_plot_plot_points_noise_aware_{}.npy'.format(hash((path,instance,d,n_iter,ep))),pltpts)
    return pltpts
...
def plot_points_noise_unaware(unaware_path,instance,n,d,n_iter,direc,ep): # Take the optimal angles at p=0, and use them to obtain an AR at other p.
    ...
    p_list=get_p_list(unaware_path,instance,d,n_iter,ep) # get the p for which I ran noise aware optimization.
    if p_list[0]!=0:
        p_list=[0]+p_list # Allways add p as a plot point.
    ...
    np.save('plot_points/lone_p_plot_plot_points_noise_unaware_{}.npy'.format(hash((unaware_path,instance,n,d,n_iter,direc,ep))),pltpts)
    return pltpts

def plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax,_color,man_p_list):
    # If you made new data points, you should remove the npy files. Only in that case the plot points are computed anew.
    fname='plot_points/lone_p_plot_plot_points_noise_aware_{}.npy'.format(hash((direc_path,instance,d,n_iter,ep)))
    ...
    ax.plot(*points,'-o',alpha=0.75,color=_color,markerfacecolor='none',markersize=7,label=label1,solid_capstyle='round')

    fname='plot_points/lone_p_plot_plot_points_noise_unaware_{}.npy'.format(hash((unaware_path,instance,n,d,n_iter,direc,ep)))
    if os.path.isfile(fname):
        points=np.load(fname)
        print('Loading plot points')
        if len(man_p_list)!=0:
...
if __name__=='__main__':
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[0],colors[1],man_p_list)
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[0],colors[2],man_p_list)
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[0],colors[0],man_p_list)
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[1],colors[1],man_p_list)
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[1],colors[2],man_p_list)
    ...
    plotter(direc_path,direc,instance,unaware_path,n_iter,d,ep,label1,label2,ax[1],colors[0],man_p_list)
    ...
    p_list_lin=[0,0.01]
    dash,=ax[1].plot(p_list_lin,[ARp0,ARp0+0.01*(-5.428360765841799)],linestyle='dashed', color='grey',markeredgecolor='black',zorder=-1,label='Linearized AR.',dash_capstyle='round') # Temp. cor.
    ax[1].plot(p_list_lin,[ARp0,ARp0+0.01*(-21.254589873784447)],linestyle='dashed', color='grey',zorder=-1,dash_capstyle='round') # Spat. cor.
    ax[1].plot(p_list_lin,[ARp0,ARp0+0.01*(-45.13075345267596)],linestyle='dashed', color='grey',zorder=-1,dash_capstyle='round') # Uncor.
