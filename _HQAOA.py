...
def get_command_line_input():
    parser = argparse.ArgumentParser(
        description="Run QAOA using the quantum emulator qem."
    )

    # Obligatory arguments
    parser.add_argument(
        "path", type=str, help="The path to the folder where output should be placed."
    )
    ...
    parser.add_argument(
        "ep",
        type=float,
        help="(Parameter proportional to) the correlation strength of the error.",
    )
    parser.add_argument(
        "direction",
        type=str,
        help="This must me set to either temporal or spatial. If temporal, errors are time-correlated. If spatial, errors are spatialy correlated.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of iterations of the basinhopping routine.",
    )
    parser.add_argument(
        "--par_mul",
        type=str,
        default="opgt",
        help="Use one parameter per cycle per gate type if set to 'opgt' (default). Use one parameter per gate instance when set to 'opgi'",
    )
    ...
    for line_nr, line in enumerate(_data):
        if line != "\n":
            try:
                ...
                print(
                    "WARNING: corruption found in line {} of {}/output.txt".format(
                        line_nr + 2, ipath
                    )
                )
...
def load_optimal_line(ipath, p, ep, d):  # not in test_HQAOA!!!!
    ...
    data = [
        line
        for line in data
        if line[0]["ep"] == ep and line[0]["p"] == p and line[0]["d"] == d
    ]
    if len(data) == 0:
...
def transition_CY_channel_classical_quantum(
    p, ep, edge
):  # Return the channel composed of 1) a classical transition channel with parameters (p,ep) on bit edge[0], 2) a CY gate on the edge, with edge[0] the control and edge[1] the target.
    re = xp.array(
        [
...
def transition_channel(p, ep, q):
    ...
    cir = qem.Cir()
    re = xp.sqrt(1 - p + ep * p) * xp.array([[1.0, 0.0], [0.0, 0.0]])
    im = xp.zeros((2, 2))
    array = qem.Array(re, im)
    ...
    if (
        ep != 1
    ):  # Condition to prevent evaluating sqrt at incorrectly rounded off argument.
        cir = qem.Cir()
        re = xp.sqrt(1 - p - ep + ep * p) * xp.array([[0.0, 1.0], [0.0, 0.0]])
        im = xp.zeros((2, 2))
        array = qem.Array(re, im)
    ...
    if p != 0 and ep != 1:  # Idem
        cir = qem.Cir()
        re = xp.sqrt(p - ep * p) * xp.array([[0.0, 0.0], [1.0, 0.0]])
        im = xp.zeros((2, 2))
        array = qem.Array(re, im)
    ...
    if not (p == 0 and ep == 0):
        cir = qem.Cir()
        re = xp.sqrt(ep + p - ep * p) * xp.array([[0.0, 0.0], [0.0, 1.0]])
        im = xp.zeros((2, 2))
        array = qem.Array(re, im)
...
def ansatz_phys_swap_sc_temporal_quantum_classical(con, n, d, p, ep, pars):
    ...
    def app_RIsingSWAP(
        edge, weight, par, psc
    ):  # Append RIsing channel to a supercir psc on the litteral qubits edge. Swap the literal qubits in edge.
        cir = qem.Cir()
    ...
    def app_phys_RIsing(
        q, par, con, psc
    ):  # Append RIsing gate on logical qubit q and q+1 to the physical super circuit. Apply the error to the right qubits.
    ...
    def app_brickwork(
        par, con, psc
    ):  # Make brickwork SWAP-network structure for evo along Ham. If  par is a list, use one parameter per gate instance.
        ipar = 0  # Needed because chainer Variables do not support pop.
        assert (
            len(par.shape) == 0 or len(par) == (n - 1) * ((n - 1) + 1) / 2
    ...
    def app_mix(
        par, psc
    ):  # Make mixer structure. If par is a list, use one parameter per gate instance.
    ...
    def app_phys_mix(
        q, par, psc
    ):  # Append mixer on logical qubit q to the supercir, containing errors.
        phys_q = q
        cir = qem.Cir()
        gate = qem.RX(par)
    ...
    for phys_q in range(n):  # Make init state
        ...
        psc.append_channel(
            qem.prepare_classical_mixed_state_channel_classical(p, phys_q + n)
        )
...
def ansatz_phys_swap_sc_temporal(con, n, d, p, ep, pars):
    ...
    def app_transitionCY(
        edge, psc
    ):  # Append transition channel to literal qubit edge[1], append CY channel to litteral qubits edge[1] (control) and edge[0] (target):
    ...
    def app_brickwork(
        par, con, psc
    ):  # Make brickwork SWAP-network structure for evo along Ham.
        for i in range(0, n, 2):  # Swap network needs n layers
            for q in range(0, n, 2):  # Even layer
...
def ansatz_phys_swap_sc_spatial(con, n, d, p, ep, pars, error_loc="all"):
    ...
    if error_loc != "all":
        ...
        assert all(
            [type(ent) == int for ent in error_loc]
        ), "All entries of error_loc must be int"
        assert (
            p == 1
        ), "You want noise to occur with certainty if you insert it at a certain location."
    ...
    def app_brickwork(
        ...
        for i in range(0, n, 2):  # Swap network needs n layers
            ...
            psc.append_channel(
                qem.prepare_classical_mixed_state_channel(p, n)
            )  # Clean up afterwards
            for q in range(1, n - 1, 2):  # Uneven layer
                app_phys_RIsingSWAP(q, par, con, psc)
...
def cost_from_parameters_swap_spatial(con, n, d, p, ep, pars, error_loc="all"):
    ...
    if error_loc != "all":
        assert (
            p == 1
        ), "If you insert an error manually, you want it to be inserted with certainty."
...
def infidelity_from_parameters_swap_temporal(con, n, d, p, ep, pars, grdsp):
    ...
    for i in range(len(grdspst)):  # Convert from +/- 1 conventon to 0/1 convention.
        for j in grdspst[i]:
            assert type(j) == int and (
                j == 1 or j == -1
            ), "Wrong entries found in the description of states in the ground space."
    ...
    for st in ord_grdspst:
        ent = (*st, *st)
        qem.round_assert(reg.psi.im.array[ent], 0.0)
...
def infidelity_from_parameters_swap_temporal_quantum_classical(
    con, n, d, p, ep, pars, grdsp
):
    """
...
def run_QAOA(cmd_args, run_args):
    ...
    def ccost(pars):  # Pars go in as numpy array.
        ...
        if cmd_args.direction == "temporal":
            cost = cost_from_parameters_swap_temporal_quantum_classical(
                run_args.con, run_args.n, cmd_args.d, cmd_args.p, cmd_args.ep, pars
            )
        elif cmd_args.direction == "spatial":
            cost = cost_from_parameters_swap_spatial(
                run_args.con, run_args.n, cmd_args.d, cmd_args.p, cmd_args.ep, pars
            )
        cost.backward()
        grad = pars.grad
        ...
        print(
            ".", end="", flush=True
        )  # Progress indicator. One dot per function call. Here one function call is defined as one forward and one backward evaluation.
    ...
    qaoa_out.init_par = (
        numpy.random.rand(cmd_args.d * 2) / 1000 - 1 / 2000
    )  #! This is truely different from

    sol = scipy.optimize.basinhopping(
        ccost,
        qaoa_out.init_par,
        minimizer_kwargs={"jac": True},
        niter=cmd_args.n_iter,
        callback=callback,
    )
...
def generate_random_SK_model_SWAP_network_graph_input(path, n):
    ...
    try:
        ...
        print(
            "!!! Attempt of creating",
            fldr_name,
            "twice!!! Prob. of this happening was low, but it should be accounted for in the weighing of results.",
        )
    ...
    print(
        ...
        fldr_name,
        "with graph input for a random SK model on",
        n,
        "spins",
    )
...
def export_Ising_ground_state(
    path,
):  # Compute the ground state of con, as defined in path/ground_state.txt, and put the result in path/graph_input.txt
    ...
    with open(path + "/graph_input.txt", "r") as file:
        ...
        assert all(
            [listq, rowq, entq, twodq, squareq]
        ), "Input file improperly formatted."
